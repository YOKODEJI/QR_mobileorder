...
-- ---- 商品オプション（トッピング等。step14→step15で商品ごとの個別設定に変更） ----
-- オプションは「その商品が持つもの」。商品ごとに名前と追加料金を設定する
-- （「ラーメンの大盛り +100円」と「カレーの大盛り +200円」を同時に持てる）。
...
-- menu_item_options: menu_items と同じ扱い（閲覧は誰でも、CRUDは自店舗スタッフのみ）。
