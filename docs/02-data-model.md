...
### menu_options / menu_item_options（商品オプション） → step14
| menu_options | | |
|---|---|---|
| id | uuid PK | |
| store_id | uuid FK | |
| name | text | 例「大盛り」。`unique(store_id, name)` |
...
| option_id | uuid FK → menu_options | `on delete cascade`。複合PK |

> **候補は店舗共通、割り当ては商品ごと**（多対多）。同じ「大盛り +100円」を複数商品で使い回せる。
> **価格はクライアントを信用しない**: 客/店員が送るのは `optionIds` だけで、追加料金は必ず `place_order` が
> `menu_options` から引く。さらに `menu_item_options` との突合で「その商品に紐付いたオプションか」も検証し、
