...
## ルーティング設計
プロトタイプは1画面にタブ同居だったが、本番は完全に分離済み（実運用は端末が別のため）。

```
app/
  layout.tsx                # フォント/globals/ルートdiv(zoom, font-family)
  page.tsx                  # ルート("/")は使わない。/admin へリダイレクトするだけ
  order/[table]/page.tsx    # 客用単独（QRの実アクセス先。管理へは行けない）
  admin/page.tsx            # 管理ツール（ログイン必須。厨房+テーブル会計+メニュー管理+設定を1つに集約）
```
...
>
> Milestone 1 は `page.tsx` に客用/管理を同居させたプロトタイプ準拠のデモ（`AppShell`）だったが、
> 本番投入時にログイン無しで管理機能へアクセスできてしまう重大な穴だったため撤去した（2026-07-13）。
> 現在の `page.tsx` は `/admin` へのリダイレクトのみ。
